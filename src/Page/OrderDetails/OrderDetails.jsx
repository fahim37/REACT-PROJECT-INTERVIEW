...
const OrderDetails = () => {
  const { purchaseData } = useContext(PurchaseFormContext);
        ...
        <div className="bg-white lg:p-p_30px w-full">
          <div className="text-center flex flex-col justify-center items-center">
            ...
            <p className="p-3 rounded-md lg:my-2 my-1 w-fit border bg-[#D2C5A2] font-bold text-lg">
              Order Id:
              <span className="font-semibold">{purchaseData.form_no}</span>
            </p>
          </div>
          <div className="w-full border flex flex-col md:flex-row md:items-start md:mt-4 mt-3 bg-[#D2C5A2] rounded-md p-4">
            <div className="md:text-base text-sm flex-1 font-semibold md:border-r-2 md:border-black md:pr-10">
              ...
              <div className="space-y-1 w-full">
                ...
                <div className="flex items-center justify-between">
                  <p>Address:</p>
                  <p>{purchaseData.present_address}</p>
                </div>
                <div className="flex items-center justify-between">
                  <p>Mobile:</p>
                  <p>{purchaseData.phone_no}</p>
                </div>
              </div>
            </div>

            <div className="md:text-base text-sm flex-1 font-semibold md:ml-10 mt-m_medium">
              <p className="font-bold md:mb-4 w-full">Course Information</p>
              <div className="space-y-1 w-full">
                <div className="flex items-center justify-between">
                  <p>Course ID:</p>
                  <p className="text-start">{purchaseData.course_id}</p>
                </div>
                <div className="flex items-center justify-between">
                  <p>Admission Date:</p>
                  <p>{purchaseData.admission_date}</p>
                </div>
                <div className="flex items-center justify-between">
                  <p>Course Fee:</p>
                  <p>{purchaseData.course_fee}</p>
                </div>
                <div className="flex items-center justify-between">
                  <p>Quantity:</p>
                  <p>{purchaseData.course_qty}</p>
                </div>
                <div className="flex items-center justify-between">
                  <p>Total Fee:</p>
                  <p>{purchaseData.total_course_fee}</p>
                </div>
              </div>
            </div>
          </div>
                ...
                <tr>
                  <td className="border text-center w-10 h-12 px-2">
                    <img
                      ...
                      src={purchaseData.photo}
                      alt={purchaseData.name}
                    />
                  </td>
                  <td className="lg:py-6 md:py-4 py-2 text-center border">
                    {purchaseData.course_name || "Course Name"}
                  </td>
                  <td className="lg:py-6 md:py-4 py-2 text-center border">
                    {purchaseData.name}
                  </td>
                  <td className="lg:py-6 md:py-4 py-2 text-center border">
                    {purchaseData.course_qty}
                  </td>
                  <td className="lg:py-6 md:py-4 py-2 text-center border">
                    {purchaseData.course_fee}
                  </td>
                  <td className="lg:py-6 md:py-4 py-2 text-center border">
                    {purchaseData.total_course_fee}
                  </td>
                </tr>
              </tbody>
            </table>
